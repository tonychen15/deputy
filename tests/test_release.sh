#!/usr/bin/env bash
# 'deputy release --marker-only [version]' inserts a parser-safe dated delimiter at the
# top of the Done section (the delimiter mechanics; the full 'deputy release' orchestrator
# is covered by test_release_full.sh). Version defaults to ./VERSION; malformed versions
# are rejected; re-releasing the same version+date is a no-op.
...
out="$(bash "$DEPUTY" release --marker-only 1.2.3 2>&1)"; rc=$?
...
setup_repo
bash "$DEPUTY" release --marker-only v3.1 >/dev/null 2>&1
...
setup_repo
printf '2.0.0\n' > "$DEPUTY_ROOT/VERSION"
out="$(bash "$DEPUTY" release --marker-only 2>&1)"; rc=$?
...
setup_repo
printf '1.0 beta\n' > "$DEPUTY_ROOT/VERSION"
bash "$DEPUTY" release --marker-only >/dev/null 2>&1; assert_eq "$?" "2" "VERSION with internal whitespace rejected (not squashed)"
...
setup_repo
bash "$DEPUTY" release --marker-only "1.0--beta" >/dev/null 2>&1; assert_eq "$?" "2" "version with '--' rejected"
bash "$DEPUTY" release --marker-only "1 0"       >/dev/null 2>&1; assert_eq "$?" "2" "version with whitespace rejected"
bash "$DEPUTY" release --marker-only "1<0>"      >/dev/null 2>&1; assert_eq "$?" "2" "version with angle brackets rejected"
...
setup_repo
bash "$DEPUTY" release --marker-only 9.9.9 >/dev/null 2>&1
out="$(bash "$DEPUTY" release --marker-only 9.9.9 2>&1)"; rc=$?

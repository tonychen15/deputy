...
assert_contains "$out" "waiting||"       "alloc: gamma has an id"
...
assert_contains "$out" "waiting||3|gamma"   "alloc: gamma gets id 3 (third added)"
...
assert_contains "$out4" "done||1|task one"     "done item keeps its id 1"
assert_contains "$out4" "waiting||2|task two"  "task two keeps id 2"
assert_contains "$out4" "task three"           "task three present"
# task three must have id > 2 (max was 2, so 3)
assert_contains "$out4" "waiting||3|task three" "new item gets id 3 (not reusing 1)"

...
run_once
assert_contains "$(bash "$DEPUTY" list)" "[P0]" "tick 1: crash → reverted to waiting (retry)"
run_once
assert_contains "$(bash "$DEPUTY" list)" "[P0]" "tick 2: still retrying (under strike limit)"

# tick 3: 3rd consecutive crash → circuit opens → surfaced (not waiting, not respawned)
run_once
out="$(bash "$DEPUTY" list)"
assert_contains "$out" "?[#"             "tick 3: circuit-breaker trips → item surfaced"
...
run_once
assert_contains "$(bash "$DEPUTY" list)" "?[#" "tick 4: surfaced item not respawned"
...
bash "$DEPUTY" list >/dev/null
run_once
assert_contains "$(bash "$DEPUTY" list)" "?[#" "startup_fail_strikes=1: trips on first crash"

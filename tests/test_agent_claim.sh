...
assert_contains "$(bash "$DEPUTY" list)" "@[#"                   "claim --agent: item transitioned to running"
...
assert_contains "$(bash "$DEPUTY" list)" "@[#"                   "recover: fresh agent claim NOT reverted"
...
bash "$DEPUTY" recover >/dev/null 2>&1
assert_contains "$(bash "$DEPUTY" list)" "[P1]"                  "recover: expired agent claim reverted to waiting"
...
assert_contains "$(bash "$DEPUTY" list)" "+[#"      "release: item transitioned to done"

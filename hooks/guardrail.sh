...
_bash_risky() {
  ...
  # `deputy cron` is allowed only if it is reschedule (the SKILL's quota failover) or the
  # read-only `status` subcommand, AND carries no ensure/remove. #88: subcommands are now
  # bare (ensure|remove|reschedule|status) with the legacy --flag forms as aliases, so
  # match BOTH the bare and `--` spellings on both the allow and block sides. Per-segment
